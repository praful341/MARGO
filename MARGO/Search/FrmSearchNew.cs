    ...
    public partial class FrmSearchNew : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void FrmSearch_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //GrdDet.OptionsFind.FindFilterColumns = SearchField;
                GrdDet.BestFitColumns();
                CustomSummary(GrdDet);
                //GrdDet.ApplyFindFilter(SearchText);
            ...
            try
            {
                if (AllowMultiSelect == false)
                {
                    if ((GrdDet.SelectedRowsCount > 0))
                    {
                        ...
                        //This is problem when open dialog then goto next control focus and fire it's click event automaticaly
                        // Start Sagar
                        //SendKeys.Send("{ENTER}");
                        // End Start Sagar
